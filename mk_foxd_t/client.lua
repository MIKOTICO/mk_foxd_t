...
local function spawnfoxd_t(c, engine)
    ...
    local model, model2, wheelModel = GetHashKey("coach4"), GetHashKey("mk_foxd_model_t"), GetHashKey("mk_wheel") -- change vehicle models here
    local attach = {0, -0.09, -0.36, 0.0, 0.0, 0.0}  -- done?
  ...
  while true do
    Citizen.Wait(0)
	  ...
	  if foxd_t then
      foxd_t.infoxd_t = IsPedInVehicle(PlayerPedId(), foxd_t.vehicle, 1)

      if foxd_t.infoxd_t then
  ...
  local exhaustOffset = { x = 0.0, y = 1.0, z = -0.29 } -- done?
  local x, y, z = table.unpack(GetEntityCoords(vehicle, false))
  ...
  local attaches = {
  [1] = {0.53, -1.29, 0.22, 180, 0.0, (90.0 + 90) % 360, "Back"}, -- done?
  [2] = {-0.43, 1.72, 0.61, -0.0, 0.0, (270.0 + 90) % 360, "Front Left"}, -- done?
  [3] = {0.44, 1.72, 0.61, -0.0, 0.0, (270.0 + 90) % 360, "Front Right"}, -- done?
  }
...
--------------------------------------------------------------------------------------------------------------------------------------------
-- Function to delete only the foxd_t.vehicle for basic locking

local function deletefoxd_tVehicle()
  if foxd_t and foxd_t.vehicle then
    DeleteEntity(foxd_t.vehicle)
  end
end
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(0)
        local player = PlayerPedId()
        local isInfoxd_tNow = (foxd_t and foxd_t.infoxd_t) or false  

        if isInfoxd_tNow and not wasInfoxd_t then  -- If the player has just entered the vehicle
            ...
            if enterVehicleCounter > 1 then  -- If player has entered more than once
                ...
                Citizen.Wait(1500)
                spawnfoxd_t(c, engine) 
            end
        end
        wasInfoxd_t = isInfoxd_tNow  -- Update the flag for the next iteration
        Citizen.Wait(250)  
    end
end)
--------------------------------------------------------------------------------------------------------------------------------------------
...
local function toggleObjectVisibility()
  if foxd_t and foxd_t.object then
    -- Toggle visibility for the main foxd_t object
    objectVisible = not objectVisible  -- Toggle the boolean
    SetEntityVisible(foxd_t.object, objectVisible)

    -- Toggle visibility for the front wheels
    if foxd_t.frontWheels then 
      for _, wheel in ipairs(foxd_t.frontWheels) do  
        SetEntityVisible(wheel, objectVisible) 
      end
    end
    
    -- Toggle visibility for the back wheels
    if foxd_t.rearWheels then 
      for _, wheel in ipairs(foxd_t.rearWheels) do  
        SetEntityVisible(wheel, objectVisible)  
      end
    end
  end
...
RegisterCommand("fixfoxd_t", function()
...
local function handleVehicleInWater()
    if foxd_t and foxd_t.vehicle then
        SetEntityVisible(foxd_t.vehicle, true) -- Make vehicle visible
        wasInWater = true
    end
end
...
local function checkVehicleInWater()
    if foxd_t and foxd_t.vehicle then
        if IsEntityInWater(foxd_t.vehicle) then
...
Citizen.CreateThread(function()
  while true do
    ...
    if DoesEntityExist(vehicle) and vehicle == foxd_t.vehicle and not DoesEntityExist(clonePed) then
      Citizen.CreateThread(function()
        Citizen.Wait(2)
    ...
    elseif (not DoesEntityExist(vehicle) or vehicle ~= foxd_t.vehicle) and DoesEntityExist(clonePed) then
...
RegisterCommand(Config.ResetCommand, function(_, args)
  if foxd_t then 
    local dist = #(GetEntityCoords(PlayerPedId()) - GetEntityCoords(foxd_t.vehicle))
    if dist < 4.0 then 
      SetEntityRotation(foxd_t.vehicle, 0.0, 0.0, 0.0, 2, 1)
    end
	end
end)
--------------------------------------------------------------------------------------------------------------------------------------------
RegisterCommand(Config.DeleteCommand, function(_, args)
    if foxd_t then 
      ...
      DeleteEntity(foxd_t.vehicle)
      DeleteEntity(foxd_t.headlight_l)
      DeleteEntity(foxd_t.headlight_r)
      DeleteEntity(foxd_t.headlight_b)
      DeleteEntity(foxd_t.object)
	  DeleteEntity(hobj)
	  DeleteEntity(hobj2)
	  DeleteEntity(hobj3)
	  DeleteEntity(foxd_t.exhaust)
	  TriggerServerEvent('stopEngineAudio', primaryAudioNetId, secondaryAudioNetId)
	  Citizen.Wait(69)
        DeleteEntity(foxd_t.primaryAudio)
        DeleteEntity(foxd_t.secondaryAudio)
      -- Delete the front wheels
      if foxd_t.frontWheels then
        for _, frontWheel in ipairs(foxd_t.frontWheels) do
          DeleteEntity(frontWheel)
        end
      end
      -- Delete the rear wheels
      if foxd_t.rearWheels then
        for _, rearWheel in ipairs(foxd_t.rearWheels) do
          DeleteEntity(rearWheel)
        end
      end
      -- Clear the foxd_t data
      foxd_t = nil
	  end
  end)
--------------------------------------------------------------------------------------------------------------------------------------------
        ...
        if foxd_t then  
            if foxd_t.infoxd_t then  
                ...
                -- Get the Network IDs for the primary and secondary audio objects
                 primaryAudioNetId = NetworkGetNetworkIdFromEntity(foxd_t.primaryAudio)
                 secondaryAudioNetId = NetworkGetNetworkIdFromEntity(foxd_t.secondaryAudio)
                
                 if IsControlPressed(0, Config.Engine) then
                 if not foxd_t.engine then  -- If the engine is not on                    if not foxd_t.engine then  -- If the engine is not on
                        print("engine on")                       
                    ...
                    foxd_t.engine = not foxd_t.engine					                   
                    Citizen.Wait(1200)
                end
            end
        else
...
Citizen.CreateThread(function()
    while true do  
        Citizen.Wait(2000)         
        if foxd_t then  
            local vehicleEntity = foxd_t.vehicle  
            ...
            if not isDriveable and foxd_t.engine then
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(10)  
        if foxd_t and foxd_t.infoxd_t and foxd_t.engine then  
...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(10)             
        if foxd_t and foxd_t.infoxd_t then                        
            local vehicleEntity = GetVehiclePedIsIn(PlayerPedId())                       
            if foxd_t.engine then  -- If the engine is on               
                if not is_particle_effect_active then
                    ...
                    if Citizen.InvokeNative(0x65BB72F29138F5D6, GetHashKey(current_ptfx_dictionary)) then  -- HasNamedPtfxAssetLoaded
                        ...
                        current_ptfx_handle_id = Citizen.InvokeNative(0x8F90AB32E1944BDE,current_ptfx_name,foxd_t.exhaust,ptfx_offcet_x,ptfx_offcet_y,ptfx_offcet_z,ptfx_rot_x,ptfx_rot_y,ptfx_rot_z,ptfx_scale,ptfx_axis_x,ptfx_axis_y,ptfx_axis_z) -- StartNetworkedParticleFxLoopedOnEntity
...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if foxd_t then     
      if foxd_t.infoxd_t then
        local speedMetersPerSecond = GetEntitySpeed(foxd_t.vehicle)
        local speedMPH = speedMetersPerSecond * 2.23694
        local curr_speed = string.format("%.1f", speedMPH) -- formatted speed       
        if foxd_t and foxd_t.engine then
          color2 = {0,126,0, 200}
        else
          color2 = {126,0,0, 200}
        end
        if foxd_t and foxd_t.headlights then
          color1 = {0,126,0, 200}
        else
          color1 = {126,0,0, 200}
        end
...
AddEventHandler('onResourceStop', function(resourceName)
    ...
    if foxd_t then 
	  ...
	  DeleteEntity(foxd_t.exhaust)
	     TriggerServerEvent('stopEngineAudio', primaryAudioNetId, secondaryAudioNetId)		
        if foxd_t.primaryAudio then
          DeleteEntity(foxd_t.primaryAudio)
        end
        if foxd_t.secondaryAudio then
          DeleteEntity(foxd_t.secondaryAudio)
        end       
        -- Delete the front wheels
        if foxd_t.frontWheels then
